...
var loadPage = function (userToken) {
    ...
    getHelp(function (items) {
        $.each(items, function (index, item) {
            var itemDiv = $("<div class='faq-card'><div class='faq-date'><h3>" + item.title +
                "</h3><span>" + item.date + "</span></div><p style='display:none;'>" + item.details + "</p></div>");
    ...
    $('.contact-tickets').click(function () {
        loadMyTickets(function (response) {
            ...
            $('.help-tickets').find('.help-search').append(table);
        });
    });
